 ...
 */
async function updateTokenIfRefreshed(igUserId, newToken, expiresIn) {
  if (!newToken) return;
  const supabase = getServerSupabaseClient();
...
export async function executePublish(accessToken, igUserId, scheduledPost, mediaUploads, { dryRun = false } = {}) {
  ...
  try {
    ...
    if (limit.newToken) {
      ...
      await updateTokenIfRefreshed(igUserId, limit.newToken, limit.expiresIn);
    }
  ...
  try {
    ...
    if (result.newToken) {
      ...
      await updateTokenIfRefreshed(igUserId, result.newToken, result.expiresIn);
    }
  ...
  try {
    ...
    if (pollResult.newToken) {
      ...
      await updateTokenIfRefreshed(igUserId, pollResult.newToken, pollResult.expiresIn);
    }
