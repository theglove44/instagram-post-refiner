...
// Map content-type to extension — derived from validated mime type, never from filename
 ...
 * Extension is derived from the validated content-type, never from the filename,
 * to prevent path traversal via crafted filenames.
 */
function generateStoragePath(scheduledPostId, contentType) {
...
export async function uploadToStorage(file, scheduledPostId) {
  ...
  const storagePath = generateStoragePath(scheduledPostId, file.type);
...
export async function uploadBufferToStorage(buffer, scheduledPostId, fileName, contentType) {
  const supabase = getServerSupabaseClient();
  const storagePath = generateStoragePath(scheduledPostId, contentType);
